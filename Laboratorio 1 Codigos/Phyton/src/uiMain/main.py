...
from Other.ErrorAplicacion import (
    ErrorSugeridoFieldFrame,
    ErrorSeleccionarDropdown,
    ErrorDineroInsuficiente,
    ErrorBusquedaInvalida,
    ErrorDepositoInvalido,
    ErrorMillasInsuficientes
)
...
com_vuelo = "Comprar Vuelo"
reas_vuelo = "Reasignar Vuelo"
cancel_vuelo = "Cancelar Vuelo"
check_in = "Check In"
gest_vuelo = "Gestion Usuario"

handlersProcesoConsulta = {
    com_vuelo: lambda mainMenu: ComprarVuelo().generar(
        mainMenu,
        com_vuelo,
        TEXT_DATA["descripcionComprarVuelo"],
    ),
    
    "reas_vuelo": lambda mainMenu: ReasignarVuelo().generar(
        mainMenu,
        "reas_vuelo",
        TEXT_DATA["descripcionReasignarVuelo"]
    ),
    
    cancel_vuelo: lambda mainMenu: CancelarVuelo().generar(
        mainMenu,
        cancel_vuelo,
        TEXT_DATA["descripcionCancelarVuelo"]
    ),
    
    check_in: lambda mainMenu: CheckIn().generar(
        mainMenu,
        check_in,
        TEXT_DATA["descripcionCheckIn"]
    ),
    
    gest_vuelo: lambda mainMenu: GestionUsuario().generar(
        mainMenu,
        gest_vuelo,
...
class MainMenu:
    ...
    def generar(self):
        ...
        menuConsultas.add_command(label=com_vuelo, command = lambda : handlersProcesoConsulta[com_vuelo](self))
        menuConsultas.add_command(label=reas_vuelo,command = lambda : handlersProcesoConsulta[reas_vuelo](self)) 
        menuConsultas.add_command(label=cancel_vuelo,command = lambda : handlersProcesoConsulta[cancel_vuelo](self))
        menuConsultas.add_command(label=check_in,command = lambda : handlersProcesoConsulta[check_in](self))
        menuConsultas.add_command(label=gest_vuelo,command = lambda : handlersProcesoConsulta[gest_vuelo](self))
    ...
    def ventana2(self, indexBoleto):
        ...
        def confirmacion(boleto):
            ...
            if (boleto.checkInRealizado):
                alertInfo(check_in, "El boleto seleccionado ya tiene check in, pasando al menu de servicios")
                ...
                if boleto.status == "Cancelado":
                    ...
                    if ok:
                        ...
                        alertInfo(check_in, "Check In realizado con exito!")
                        self.ventanaServicios(boleto)

    def ventanaServicios(self, boleto):
        self.clearZone()
        self.zona3 = self._crearZona(self.zonaForm, 2)

        # Mostrar info del boleto
        ...
        # Dropdown opciones
        opciones = ["Mejorar asiento", "Comprar servicios especiales"]
        dropDown = self._crearDropdown(
            infoServicios.marco, "Seleccionar opcion", opciones, nextFreeRow
        )
        separador = getSeparador(infoServicios.marco, nextFreeRow + 1, 2)
        self.zonaResult = self._crearZona(self.zonaForm, 1)

        handlers = {
            "Mejorar asiento": lambda row, b: self._mejorarAsiento(row, b),
            "Comprar servicios especiales": lambda row, b: self._comprarServicios(row, b),
        }
        dropDown.bind("<<ComboboxSelected>>", lambda e: handlers[dropDown.get()](nextFreeRow + 2, boleto))

    def _crearZona(self, parent, row):
        frame = tk.Frame(parent, bg=color["pink"], highlightbackground="#9656B6", highlightthickness=2)
        frame.grid(row=row, column=0, sticky="ew", padx=5, pady=5)
        return frame

    def _crearDropdown(self, parent, label, options, row):
        tk.Label(parent, text=label, font=("fixedsys", 12), bg="#E1BEFF").grid(row=row, column=0, padx=5, pady=5)
        dropdown = ttk.Combobox(parent, state="readonly", values=options, font="fixedsys")
        dropdown.grid(row=row, column=1, padx=15, pady=15)
        return dropdown

    def _mejorarAsiento(self, row, boleto):
        self._resetZona(1)
        self._resetZona(2)
        vip_asientos = [a for a in boleto.vuelo.asientos if a.tipo == "Vip"]

        drop = self._crearDropdown(self.zonaResult, "Seleccionar nuevo asiento", vip_asientos, row)
        getBotonTemp(self.zonaResult, lambda: self.cancel(), row + 1, 0)
        getBotonContinuar(
            self.zonaResult,
            lambda: self._confirmarMejora(boleto, vip_asientos[drop.current()]) if drop.current() != -1 else alertWarn("Error", "Seleccione un asiento"),
            row + 1,
            1,
        )

    def _confirmarMejora(self, boleto, asiento):
        if boleto.tipo == "Vip":
            alertWarn("Error", "Ya es VIP")
            return
        if user.dinero < 35:
            alertWarn("Error", "Fondos insuficientes")
            return
        if alertConfirmacion("¿Confirmar mejora de asiento por $35?"):
            boleto.upgradeAsiento(asiento)
            alertInfo("OK", "Mejora exitosa")
            self.clearZone()
            self.ventanaServicios(boleto)

    def _comprarServicios(self, row, boleto):
        self._resetZona(2)
        servicios = [
        ...
        drop = self._crearDropdown(self.zonaResult, "Seleccionar servicio", servicios, row)
        drop.bind("<<ComboboxSelected>>", lambda e: self._handleServicio(drop.get(), boleto, row + 1))

    def _handleServicio(self, tipo, boleto, row):
        self._resetZona(2)
        servicios_func = {
            "Comida a la carta": lambda: self._compraSimple(ServiciosEspeciales.COMIDA_A_LA_CARTA, boleto, row),
            "Viaje con mascota": lambda: self._compraMascota(boleto, row),
            "Acompañante para menor de edad": lambda: self._compraSimple(ServiciosEspeciales.ACOMPANANTE_PARA_MENOR, boleto, row),
            "Asistencia para pasajero con necesidades especiales": lambda: self._compraSimple(ServiciosEspeciales.ASISTENCIA_NECESIDADES_ESPECIALES, boleto, row),
            "Transporte terrestre": lambda: self._compraSimple(ServiciosEspeciales.TRANSPORTE_TERRESTRE, boleto, row),
            "Ver servicios contratados": lambda: self._mostrarServiciosContratados(boleto),
        }
        servicios_func[tipo]()

    def _compraSimple(self, servicio, boleto, row):
        if alertConfirmacion(f"¿Desea contratar este servicio por ${servicio.precio}?"):
            if user.dinero >= servicio.precio:
                boleto.comprarServicio(servicio)
                alertInfo("OK", "Servicio contratado")
                self.clearZone()
                self.ventanaServicios(boleto)
            else:
                alertWarn("Error", "Fondos insuficientes")

    def _compraMascota(self, boleto, row):
        def confirmar(data):
            tipo = data["Perro/Gato"].lower()
            if tipo == "perro":
                mascota = Perro(data["Nombre"], data["Raza"], float(data["Peso"]))
            elif tipo == "gato":
                mascota = Gato(data["Nombre"], data["Raza"], float(data["Peso"]))
            else:
                alertWarn("Error", "Solo se acepta perro o gato")
                return
            if user.dinero >= ServiciosEspeciales.MASCOTA_EN_CABINA.precio:
                boleto.comprarServicioMascota(mascota)
                alertInfo("OK", f"{mascota.nombre} viajará contigo")
                self.clearZone()
                self.ventanaServicios(boleto)
            else:
                alertWarn("Error", "Fondos insuficientes")
        FieldFrame("Datos mascota", ["Nombre", "Raza", "Peso", "Perro/Gato"], "Datos", None, None, self.zona3, callback=confirmar)

    def _mostrarServiciosContratados(self, boleto):
        data = {f"Servicio #{i+1}": s for i, s in enumerate(boleto.serviciosContratados)} or {"Servicios": "No hay servicios contratados"}
        ResultFrame("Servicios contratados", data, self.zona3)

    def _resetZona(self, row):
        setattr(self, f"zona{row}", self._crearZona(self.zonaForm, row))
...
class GestionUsuario(VentanaBaseFuncionalidad):
    ...
    def ventanaHistorial(self):
        ...
        def handleBuscarVuelo(formData):
            ...
            except ErrorBusquedaInvalida:
                alertWarn("Error", "Error, número de vuelo inválido, no se encontraron resultados para la búsqueda")
            except ValueError:
                alertWarn("Error", "Valor inválido ingresado")
            except Exception as e:
                alertWarn("Error inesperado", f"Ocurrió un error inesperado: {e}")
    ...
    def ventanaDepositar(self, valor):
        try:
            ...
            alertInfo("Depósito realizado con éxito", f"Se ha agregado ${valor:.2f} a tu cuenta, nuevo saldo: {user.dinero:.2f}")
            self.cancel()

        except ValueError:
            alertWarn("Error", "El valor ingresado no es un número válido")

        except ErrorDepositoInvalido:
            alertWarn("Error", "El monto a depositar debe ser mayor a cero")

        except Exception as e:
            alertWarn("Error inesperado", f"Ocurrió un error: {e}")
